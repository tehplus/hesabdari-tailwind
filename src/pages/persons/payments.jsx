...
export default function PersonsPaymentList() {
  const [payments, setPayments] = useState(initialPayments);
  const [searchTerm, setSearchTerm] = useState('');

  const handleDelete = async (id) => {
    const confirmed = await showConfirm('حذف پرداخت', 'آیا از حذف این پرداخت اطمینان دارید؟');
    if (confirmed) {
      setPayments(payments.filter(payment => payment.id !== id));
      showSuccess('پرداخت مورد نظر با موفقیت حذف شد');
    }
  };
  ...
  const filteredPayments = payments.filter(payment => 
    payment.person.includes(searchTerm) || 
    ...
    <div className="space-y-6">
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <h1 className="text-xl font-semibold text-gray-900">لیست پرداخت‌ها</h1>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-300">
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredPayments.map((payment) => (
                <tr key={payment.id}>
                  <td className="whitespace-nowrap py-4 pr-4 text-sm font-medium text-gray-900">{payment.person}</td>
                  ...
                  <td className="whitespace-nowrap px-3 py-4 text-sm">
                    <span className={`inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${getStatusBadgeColor(payment.status)}`}>
                      {getStatusText(payment.status)}
                    </span>
                  </td>
                  <td className="relative whitespace-nowrap py-4 pl-3 text-right text-sm font-medium">
                    <Link
                      to={`/persons/payments/${payment.id}/edit`}
